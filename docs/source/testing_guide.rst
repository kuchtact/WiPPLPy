...
To prepare to run tests from the command line, :ref:`open a terminal <opening-a-terminal>` and
install `Nox`_:
...
This command will invoke `pytest` to run WiPPLPy's tests, excluding the
tests marked as slow.

Writing tests
-------------

* All functionality in WiPPLPy must have tests.

* Tests are located in the top-level :file:`tests/` directory. For example,
...
Every code contribution to WiPPLPy with new functionality must also
have corresponding tests. Creating or updating a pull request will
activate WiPPLPy's test suite to be run via `GitHub Actions`_, along
...
does it in isolation from other tests. A typical
unit test is broken up into three parts: *arrange*, *act*, and
*assert*. An integration test verifies that
multiple software components work together as intended.

WiPPLPy's tests are run using `pytest` and `Nox`_. Tests are located in
the :file:`tests/`_ directory. For example, tests of `wipplpy.modules`
are located in :file:`tests/modules` and tests of
`wipplpy.modules.shot_loader` are located in
:file:`tests/modules/test_shot_loader.py`.
...
Tests are located in the top-level :file:`tests/`_ directory. The directory
structure of :file:`tests/`_ largely mirrors that of :file:`src/wipplpy/`, which
contains the source code of WiPPLPy.

The tests of a subpackage named :samp:`wipplpy.{subpackage}` are
located in the :samp:`tests/{subpackage}/` directory. Tests for a module
named :samp:`wipplpy.{subpackage}.{module}` are generally located in
:samp:`tests/{subpackage}/test_{module}.py`. For example, tests for
`wipplpy.modules` are located in :file:`tests/modules`, and tests
of `wipplpy.modules.shot_loader` are located in
:file:`tests/modules/test_shot_loader.py`.

Test functions within each file have names that begin with `test_`
and end with a description of the behavior that is being tested. For
example, a test to checks that a :class:`~wipplpy.modules.generic_get_data.Data` can load data from a file
might be names `:test_load_diagnostic_data_from_file`.
Because :class:`~wipplpy.modules.generic_get_data.Data` is defined in
:file:`src/wipplpy/modules/generic_get_data.py`, this test would be
located in :file:`tests/modules/test_generic_get_data.py`.
...
`test_`. For example, :file:`test_connection.py` could define a
`TestConnection` class containing the method ``test_shot_number``.
...
``truth_value`` in `[True, False]`. Both of the above tests will be
...
`list` (commonly referred to as `args`) and keyword arguments in a
`dict` (commonly referred to as `kwargs`). Unpacking_ occurs when
...
* **Turn bugs into test cases**. It is said that
  "every every bug exists because of a missing test"
  . After finding a bug, write a minimal failing
...
* **Make tests deterministic.** When a test fails intermittently, it is
  ...
  .. tip::
     ...
     `@pytest.mark.flaky` decorator from `pytest-rerunfailures`_
...
* If the *act* phase of a unit test is more than a single line of
  ...
  be called in a single line of code.
...
command line using `pytest` or `Nox`_, or via an IDE. In particular,
using `Nox`_ ensures that tests are run in the same way as in CI.
...
* Checks with labels like **CI / Python 3.x (pull request)** verify that
  ...
  up using `Nox`_ and run with `pytest` via `GitHub Actions`_. When
...
(including `Nox`_ and pytest_), run:
...
operating systems. Testing with `Nox`_ is more robust than testing with
`pytest` alone because `Nox`_ creates its own virtual environments and
...
To find out what `Nox`_ sessions are defined, run:
...
where ``<session>`` is replaced with the name of the `Nox`_ session. The
...
Commands using `Nox`_ must be run in the top-level directory of the
